...
namespace JayCadSurveyXamarin.Model
{
    ...
    public class Angle
    {
		...
		private void ConvertDegMinSecToDecimal()
		{
			...
			_decimalAngle = _degrees + ((double)_minutes / 60) + ((double)_decimalSeconds / 3600);
		...
		private void ConvertDecimalToDegMinSec()
		{
			...
			double calcMinutesValue = ((_decimalAngle - (double)degrees) * 60);
		...
		public String addAngle(Angle otherAngle)
		{
			...
			return _degrees + "\u000B0" + Math.Abs(_minutes) + "' " + Math.Abs(_seconds) + "\"";
		}
